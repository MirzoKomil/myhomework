/**
 * homeworkuz.uz — mavjud inline <script> blokining TO'LIQ o'rniga qo'yiladigan tuzatilgan versiyasi.
 *
 * Nima uchun kerak (2 ta muammo topildi, ikkalasi ham tasdiqlangan):
 *
 * 1) KRITIK BUG: "Ingliz tilini boshlash" tugmasi bosilganda skript
 *    `input[name="plat"]` va `#e3` elementlarini qidiradi — lekin bu
 *    element sahifada UMUMAN YO'Q (ehtimol avval bo'lgan 3-savol keyinchalik
 *    HTML'dan olib tashlangan, lekin validatsiya kodi yangilanmagan).
 *    Natijada `document.getElementById('e3').classList...` — `null` ustida
 *    chaqirilib, JS xatosi (TypeError) tashlaydi va tugma HECH QACHON
 *    muvaffaqiyatli bosilmaydi — hozirgi holatda saytdagi forma orqali
 *    HECH KIM ro'yxatdan o'ta olmaydi (na Telegramga yo'naltiriladi, na
 *    boshqa hech narsa sodir bo'ladi). Bu haqiqiy brauzer orqali sinab
 *    ko'rilib tasdiqlandi.
 *
 * 2) Forma ma'lumotlari (ism, telefon, qulay vaqt, maqsad) hech qayerga
 *    yuborilmasdi — faqat validatsiyadan o'tgach 700ms'dan keyin
 *    https://t.me/homeworkeng_bot ga bo'sh (hech qanday ma'lumotsiz)
 *    yo'naltirilardi. Endi shu ma'lumotlar CRM'ning tayyor
 *    /api/leads webhookiga ham yuboriladi — Sotuv bo'limining
 *    "Yangi lidlar" ustuniga avtomatik tushadi (til = ingliz tili).
 *
...
document.getElementById('submit-btn').addEventListener('click', () => {
  ...
  // 46-vazifa (fix): "plat" guruhi HTML'da mavjud emas — shu tekshiruv olib tashlandi (aynan shu joy xato tashlab, formani butunlay ishlamay qo'yayotgan edi).
  ...
  if (ok) {
    ...
    // CRM'ning insertLead()'i "contactTime"dan boshqa erkin maydon
    // qabul qilmaydi (masalan "maqsad" uchun alohida ustun yo'q) — shu
    // sabab ikkalasi bitta izoh matniga birlashtirilib yuboriladi, CRM
    // buni lidning birinchi izohi sifatida avtomatik saqlaydi.
    const noteText = `Qulay vaqt: ${timeLabel} | Maqsad: ${goalLabel}`;

    // CRM'ning Sotuv > Yangi lidlar ustuniga yuboriladi (til: ingliz tili).
    fetch(CRM_API_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Webhook-Secret': CRM_WEBHOOK_SECRET,
        'X-Lead-Source': 'homework',
      },
      body: JSON.stringify({
        name: nm,
